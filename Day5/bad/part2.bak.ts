...
function convert(ids:range, fromType, toType, conversions):range[]|null {
	...
	let mapped = sources.map((source, sourceInd):range|undefined => {
		...
		//let intersectionStart, intersectionEnd; // indices on source
	...
	while (residues.length > 0) {
		...
		sources.forEach((source:range) => {
			...
			// see which parts of the residue aren't in the source
...
// }))
	...
	const targetRanges = state.source.map(sourceIds => {
		return convert(sourceIds, from, to, conversions)
